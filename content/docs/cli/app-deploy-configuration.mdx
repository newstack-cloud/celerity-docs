...
App deploy configuration is used for blueprint variable overrides and configuration for the target environment to deploy the application to. This configuration will be converted to the Bluelink [deploy configuration](https://bluelink.dev/docs/cli/deploy-configuration) format when an application is built and deployed.
