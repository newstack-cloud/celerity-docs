---
title: Celerity Config
description: Celerity Config resource type
---
...
- **v0 (Current)**: AWS only, supporting AWS Secrets Manager and AWS SSM Parameter Store.
- **v1 (Preview)**: Multi-cloud, supporting Google Cloud Secret Manager and Azure Key Vault in addition to AWS Secrets Manager and AWS SSM Parameter Store.
...
🚀 **Planned for v1** - Secret rotation functionality may become available in a future v0 evolution.
...
In the local development environment, secrets and configuration are stored as a key/value pair in a [valkey](https://github.com/valkey-io/valkey) instance running on a container network or directly on the host for a local or CI machine.
The value will be the JSON-encoded string holding all the secrets and configuration values for the application.
In the local development environment, the same valkey instance will be reused for config stores and cache resource types.
Replication and encryption configuration will be ignored in the local development environment.
Rotation configuration will also be ignored in the local development environment, secrets will not be rotated automatically.
...
Configuration specific to a target environment can be defined for `celerity/config` resources in the [app deploy configuration](/docs/cli/app-deploy-configuration) file.
