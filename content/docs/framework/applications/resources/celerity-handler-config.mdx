---
title: Celerity Handler Config
description: Celerity resource type for shared handler configuration
---
...
The [`metadata.sharedHandlerConfig`](/docs/framework/applications/resources/celerity-handler#sharing-handler-configuration) approach may be preferred when you want all handlers in the blueprint to share the same default configuration.
</Callout>
...
Handler config resources can be linked from multiple handler
resources to share configuration between them.
...
See the [runtimes](/docs/framework/applications/resources/celerity-handler#runtimes) section of the `celerity/handler` resource type documentation for a list of supported runtimes.
