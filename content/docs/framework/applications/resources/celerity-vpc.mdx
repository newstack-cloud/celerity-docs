...
- **`private`** (default): the function can reach private resources in the VPC (such as a linked `celerity/sqlDatabase` or `celerity/cache`) and, where the VPC provides a NAT gateway, the public internet.
