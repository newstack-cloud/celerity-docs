---
title: Celerity Workflow
description: Celerity resource type for workflows
---
...
- **v1 (Preview)**: Multi-cloud with support for deployment as containerised applications as well as serverless workflow deployments across all cloud providers and all language runtimes.

Read more about Celerity versions [here](/docs/framework/versions).
</Callout>

🚀 **Planned for v1** - Workflow support is planned for v1 with multi-cloud and containerised deployment support across all cloud providers and all language runtimes.
...
<FieldHeading fieldName="celerity.workflow.vpc.subnetType" />
...
<FieldHeading fieldName="celerity.workflow.vpc.lbSubnetType" />
...
<FieldHeading fieldName="type (required)" />
...
<FieldHeading fieldName="inputPath" />
...
<FieldHeading fieldName="outputPath" />
...
<FieldHeading fieldName="payloadTemplate" />
...
<FieldHeading fieldName="next (conditionally required)" />
...
<FieldHeading fieldName="decisions (conditionally required)" />
...
<FieldHeading fieldName="timeout" />
...
<FieldHeading fieldName="waitConfig (conditionally required)" />
...
<FieldHeading fieldName="failureConfig (conditionally required)" />
...
<FieldHeading fieldName="parallelBranches (conditionally required)" />
...
<FieldHeading fieldName="and (conditionally required)" />
...
<FieldHeading fieldName="or (conditionally required)" />
...
<FieldHeading fieldName="not (conditionally required)" />
...
<FieldHeading fieldName="condition (conditionally required)" />
...
<FieldHeading fieldName="inputs (required)" />
...
```blueprintlang
inputs = ["$.result.id", "someConstant"]
```

```blueprintlang
inputs = [594.34, false, none]
```
...
<FieldHeading fieldName="function (required)" />
...
<FieldHeading fieldName="startAt (required)" />
...
<FieldHeading fieldName="states (required)" />
...
<Callout title="State names" type="warning">
State names must be unique within the entire workflow definition so that handlers can be wired up to the correct states via [workflow to handler link annotations](/docs/framework/applications/resources/celerity-handler#celerityworkflow--celerityhandler).
</Callout>
...
<FieldHeading fieldName="seconds (conditionally required)" />
...
<FieldHeading fieldName="timestamp (conditionally required)" />
...
```blueprintlang
matchErrors = ["Timeout", "ResourceNotFound"]
```

```blueprintlang
matchErrors = ["*"] # Retry on all errors
```
...
<FieldHeading fieldName="interval" />
...
<FieldHeading fieldName="maxAttempts" />
...
<FieldHeading fieldName="maxDelay" />
...
<FieldHeading fieldName="backoffRate" />
...
Handlers are used to execute steps in a workflow. In the context of a workflow, a step is the state of the `executeStep` type. The handler is configured with [annotations](/docs/framework/applications/resources/celerity-handler#celerityworkflow--celerityhandler) that determine the state in the workflow that will trigger the handler.
...
resource videoIngestWorkflow: celerity/workflow {
    select by label {
...
resource downloadVideoHandler: celerity/handler {
    metadata {
...
resource scanVideoHandler: celerity/handler {
    metadata {
...
resource processVideoHandler: celerity/handler {
    metadata {
...
resource docProcessingWorkflow: celerity/workflow {
    select by label {
        ...
        states = {
            ...
            failureDecision = {
                ...
                decisions = [
                    ...
                    {
                        or = [
                            { function = "eq", inputs = ["$.error", "ExtractPDFError"] },
                            { function = "eq", inputs = ["$.error", "PDFLoadError"] }
                        ],
                        next = "processPDFFailure"
                    },
                    {
                        or = [
                            { function = "eq", inputs = ["$.error", "ExtractDOCXError"] },
                            { function = "eq", inputs = ["$.error", "DOCXLoadError"] }
                        ],
...
resource fetchDocumentHandler: celerity/handler {
    metadata {
...
resource scanDocumentHandler: celerity/handler {
    metadata {
...
resource processPDFHandler: celerity/handler {
    metadata {
...
resource processDOCXHandler: celerity/handler {
    metadata {
...
resource systemUploadHandler: celerity/handler {
    metadata {
...
In the local development environment, a workflow is deployed as a containerised version of the Celerity workflow runtime in a local container orchestrator.
...
In the local environment, the runtime is backed by a [SQLite](https://www.sqlite.org/) database for storing workflow execution state.
...
Workflows can be deployed to [ECS](https://aws.amazon.com/ecs/) or [EKS](https://aws.amazon.com/eks/) backed by [Fargate](https://aws.amazon.com/fargate/) or [EC2](https://aws.amazon.com/ec2/) using [compute configuration](/docs/framework/applications/compute-configuration#aws-configuration-options) that is shared between compute resource types in the app deploy configuration file.
...
If backed by EC2, the auto-scaling group will scale the number of instances based on CPU utilisation of the instances with a minimum of 1 instance and a maximum of `N` instances depending on the [app environment](/docs/cli/app-deploy-configuration#structure).
...
Workflows can be deployed to [Cloud Run](https://cloud.google.com/run), as well as [Google Kubernetes Engine (GKE)](https://cloud.google.com/kubernetes-engine) in [Autopilot](https://cloud.google.com/kubernetes-engine/docs/concepts/autopilot-overview) or [Standard](https://cloud.google.com/kubernetes-engine/docs/how-to/creating-a-regional-cluster) mode using [deploy configuration](/docs/framework/applications/compute-configuration#google-cloud-configuration-options) for the Google Cloud target environment.

The workflow runtime requires persistence for storing workflow execution state, this is provided by a set of Firestore documents that are created when the workflow is deployed.

<Callout title="Note" type="info">
Firestore was chosen as the persistence layer for the workflow runtime in Google Cloud environments as it provides a scalable and highly available NoSQL database that is more than sufficient for the requirements of the workflow runtime.
This also removes the need to manage a relational database cluster for each workflow-based application that is deployed.
</Callout>
...
The workflow runtime uses the [NoSQL API for Cosmos DB](https://learn.microsoft.com/en-us/azure/cosmos-db/nosql/).
...
Workflows can be deployed to Azure Logic Apps using [deploy configuration](/docs/cli/app-deploy-configuration) for the Azure Serverless target environment.

## App Deploy Configuration

Configuration specific to a target environment can be defined for `celerity/workflow` resources in the [app deploy configuration](/docs/cli/app-deploy-configuration) file.
...
Compute configuration that can be used for the `celerity/api`, `celerity/consumer`, `celerity/schedule` and the `celerity/workflow` resource types is documented [here](/docs/framework/applications/compute-configuration).
...
- Celerity workflows only support handlers of the `celerity/handler` resource type for the `executeStep` state type. For integrations with cloud provider services, you will either need to use the cloud provider's workflow service or use a handler that can interact with the cloud provider's API.
