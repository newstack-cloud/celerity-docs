---
title: Celerity Schedule
description: Celerity resource type for scheduled triggers
---
...
- **v0 (Current)**: AWS only, supporting AWS EventBridge scheduled rules to trigger Lambda functions.
- **v1 (Preview)**: Multi-cloud with support for cloud provider schedulers triggering containerised and FaaS applications.
...
A static JSON value that is delivered to the schedule handler on every trigger.
This can be used to pass configuration data, task identifiers, or any context that
the handler needs to determine what work to perform.
...
<FieldHeading fieldName="celerity.schedule.vpc.subnetType" />

🚀 **Planned for v1** - This annotation will be applicable when schedules can be deployed to a containerised environment, it may become available in a future v0 evolution.
...
<FieldHeading fieldName="celerity.schedule.vpc.ingressType" />
...
For example, if the resource name is `mySchedule`, the output would be accessible via `${mySchedule.spec.queueId}`.

### queueId (optional)

The ID of the queue or pub/sub topic that is created for the schedule.
This output is **only** present in the outputs when the target environment requires a queue or pub/sub topic as an intermediary between the scheduler and the scheduled handler (for example, container-backed AWS, GKE or Azure targets). It is not present for serverless target environments, where the scheduler triggers the handler directly.
...
🚀 **Planned for v1** - This link will be applicable when schedules can be deployed to a containerised environment, it may become available in a future v0 evolution.
...
resource jobSchedule: celerity/schedule {
    metadata {
...
In the local development environment, a schedule application is deployed as a containerised version of the Celerity runtime that consumes a stream from Valkey as per the behaviour defined in the [`celerity/consumer`](/docs/framework/applications/resources/celerity-consumer#local-development) resource type.
...
Schedule applications can be deployed to [ECS](https://aws.amazon.com/ecs/) or [EKS](https://aws.amazon.com/eks/) backed by [Fargate](https://aws.amazon.com/fargate/) or [EC2](https://aws.amazon.com/ec2/) using [deploy configuration](/docs/cli/app-deploy-configuration) for the AWS target environment.
...
Schedule applications can be deployed to Azure Functions with timer triggers using [deploy configuration](/docs/cli/app-deploy-configuration) for the Azure Serverless target environment.
...
input                  | Delivered in the message body to the handler              | Delivered in the message body to the handler | Delivered in the message body to the handler
...
Configuration specific to a target environment can be defined for `celerity/schedule` resources in the [app deploy configuration](/docs/cli/app-deploy-configuration) file.
