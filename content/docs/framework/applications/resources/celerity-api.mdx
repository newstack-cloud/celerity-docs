...
Containerised environments and custom servers use the Celerity runtime for the language of choice to be deployed to handle incoming requests and route them to your [application handlers](/docs/framework/applications/resources/celerity-handler).
The [Bluelink deploy engine](https://bluelink.dev/docs/bluelink/overview) and platforms built on top of it will take care of this for the supported target environments (e.g. Amazon ECS).
...
On the AWS Serverless target, API Gateway v2 (HTTP and WebSocket) APIs do not support X-Ray active tracing at the gateway/stage level (X-Ray active tracing is only available on API Gateway REST/v1 stages). Enabling `tracingEnabled` therefore does not enable stage-level tracing; instead, enable tracing on the linked handlers (`celerity/handler` `tracingEnabled`), whose Lambda functions are traced with X-Ray and produce the request traces.
...
For custom server or containerised environments, this will be the same as the `baseUrl` output field.
...
When `connect` is set, the client is expected to send the authentication token in the connection request via a HTTP header or cookie. Browser clients must use cookies as the browser `WebSocket` API does not support custom headers on the handshake request.
...
The name of the auth guard (or ordered list of guards) in the API's auth guard map to use for every new WebSocket connection established by clients.
This is required when the `authStrategy` field is set.

When multiple guards are specified as an array, they execute in order and all must pass.
Claims from all passing guards are accumulated and namespaced by guard name.
...
```blueprintlang
basePaths = [
    { protocol = "http", basePath = "/api" },
    { protocol = "websocket", basePath = "/ws" }
]
```
...
The identifier of the guard (or ordered list of guards) that should be used as the default for the API.
This should only be set when you want to use the same guard(s) for all endpoints of the API.
Individual handlers can opt out by setting the `celerity.handler.public` annotation to `true`.

When multiple guards are specified as an array, they execute in order and all must pass.
If any guard fails, the request is rejected immediately with that guard's error response.
...
When running an API in the Celerity runtime (containerised deployments), the `oauth2` discovery mode will always be supported, meaning the `/.well-known/oauth-authorization-server` will be used to retrieve the discovery document.
</Callout>
...
Note that browser clients **cannot** send custom headers on the WebSocket handshake request, so `$.cookies.*` is the only viable token source for browser-based `connect` auth. Currently, only a single token source is supported per protocol, so the application must choose one transport mechanism: use `$.cookies.*` for browser-facing APIs or `$.headers.*` for server-to-server APIs. See the [WebSocket Auth Strategy](#websocket-auth-strategy) section and the [WebSocket Runtime Protocol](/docs/framework/runtime/websocket-runtime-protocol#connect-approach) for more details.
</Callout>
...
```blueprintlang
# authMessage strategy: token in the WebSocket message body
tokenSource = [
    { protocol = "http", source = "$.headers.Authorization" },
    { protocol = "websocket", source = "$.data.token" }
]
```

```blueprintlang
# connect strategy: cookie-based token for browser clients
tokenSource = [
    { protocol = "http", source = "$.headers.Authorization" },
    { protocol = "websocket", source = "$.cookies.authToken" }
]
```
...
Note that browser clients **cannot** send custom headers on the WebSocket handshake request, so `$.cookies.*` is the only viable source for browser-based `connect` auth. Only a single token source is supported per protocol, so the application must choose one transport mechanism.
</Callout>
...
When an API with the WebSocket protocol links to a channel, the channel layer is activated for that API. The API's `linkSelector` selects channel resources by label, the same way it selects handlers. Without any linked channel resources, the channel protocol routes (`channel:subscribe`, `channel:unsubscribe`, etc.) are not registered and the runtime has zero channel overhead.

Channels provide real-time pub/sub messaging, presence tracking, per-channel auth, and rate limiting on top of the WebSocket API.

🔮 **Planned for v1+**

[Read more about Celerity channels](/docs/framework/applications/resources/celerity-channel)
...
resource orderStreamApi: celerity/api {
    metadata {
    ...
    spec {
        protocols = [
            { websocketConfig = { routeKey = "action" } }
        ]
        cors = {
            allowOrigins = [
                "https://example.com",
...
### WebSocket API (`connect` auth strategy)
        ...
        auth = {
            ...
            guards = {
                jwt = {
                    ...
                    issuer = "https://identity.newstack.cloud/oauth2/v1/",
                    tokenSource = [
                        # Cookie for browser clients connecting via the connect strategy
                        { protocol = "websocket", source = "$.cookies.authToken" }
                    ],
                    audience = ["https://identity.newstack.cloud/api/manage/v1/"]
                }
...
<Callout title="Browser clients and the connect strategy" type="info">
The browser `WebSocket` API does not support custom headers on the handshake request. When browser clients use the `connect` strategy, the auth token must be sent via a cookie. Configure `$.cookies.*` as the WebSocket token source for browser-facing APIs, or `$.headers.*` for APIs that only serve server-side or native clients.

See the [WebSocket Runtime Protocol](/docs/framework/runtime/websocket-runtime-protocol#connect-approach) for more details.
</Callout>
...
- `0x5` - This route is reserved for the server capabilities signal. The server sends this binary frame (`[0x1 0x5 0x0 0x0]`) immediately after a WebSocket connection is established to indicate that the server supports full protocol capabilities including binary messages, custom close codes, and binary control frames (acks and lost-message notifications). In environments where binary frames are not supported (e.g., managed WebSocket gateways), this frame will not reach the client, and the client should assume constrained capabilities. See the [WebSocket Runtime Protocol - Server Capabilities](/docs/framework/runtime/websocket-runtime-protocol#server-capabilities) section for full details.
...
The token configured in the API's auth guard token source must be present in the WebSocket upgrade request. The server extracts the token and authenticates it before upgrading the connection.

**Token transport depends on the client environment:**

- **Server-side or native clients** (Node.js, Python, mobile apps, CLI tools) can send the token as a HTTP header (e.g. `Authorization: Bearer <token>`) on the upgrade request.
- **Browser clients** cannot send custom headers on the WebSocket handshake as the browser `WebSocket` API does not support this. In this environment, the token must be sent via a **cookie**. The application's HTTP login or token exchange endpoint should set an `HttpOnly`, `Secure`, `SameSite=Strict` (or `SameSite=Lax`) cookie containing the auth token, which the browser will automatically include on the upgrade request.

<Callout title="Cross-site browser clients" type="warning">
Cookie-based auth requires the WebSocket API to be **same-site** as the client application (e.g. `app.example.com` connecting to `api.example.com`). For same-site, different subdomain setups, set the cookie with `Domain=.example.com` so it is included on the upgrade request.

For **cross-site** deployments (e.g. `example.com` connecting to `different.com`), cookies with `SameSite=Strict` or `SameSite=Lax` will not be sent on the upgrade request. Since browsers also cannot set custom headers on the WebSocket handshake, the `connect` strategy is not viable for cross-site browser clients. Use the `authMessage` strategy instead.
</Callout>

Configure the [`tokenSource`](#authguardconfiguration) to match the client environment. For browser-facing APIs, use a cookie source:
...
For APIs that only serve server-side or native clients, use a header source:
...
tokenSource = [
    { protocol = "http", source = "$.headers.Authorization" },
    { protocol = "websocket", source = "$.headers.Authorization" }
]
```
...
<Callout title="Why not query parameters?" type="warning">
While the token source supports `$.query.*`, sending tokens as query parameters (e.g. `wss://example.com/ws?token=...`) is **discouraged** because URLs may be logged by proxies, load balancers, and server access logs, exposing the token. Cookie-based transport avoids this risk.
</Callout>

See the [WebSocket Runtime Protocol](/docs/framework/runtime/websocket-runtime-protocol#connect-approach) for the full protocol details including the connection lifecycle and failure handling.
...
In the local development environment, an API is deployed as a containerised version of the Celerity runtime in Docker.
...
APIs can be deployed to [ECS](https://aws.amazon.com/ecs/) or [EKS](https://aws.amazon.com/eks/) backed by [Fargate](https://aws.amazon.com/fargate/) or [EC2](https://aws.amazon.com/ec2/) using [compute configuration](/docs/framework/applications/compute-configuration#aws-configuration-options) that is shared between compute resource types in the app deploy configuration file.
...
You can use [compute configuration](/docs/framework/applications/compute-configuration#google-cloud-configuration-options) shared between compute resource types in the app deploy configuration file to fine-tune the compute configuration for your application.
...
Compute configuration that can be used for the `celerity/api`, `celerity/consumer`, `celerity/schedule` and `celerity/workflow` resource types is documented [here](/docs/framework/applications/compute-configuration).
...
When set to `true`, OpenTelemetry will be enabled for Azure Functions. This feature is currently in preview (July 2026) and is only supported for a subset of languages.
...
- OAuth2 discovery to retrieve the JWKS (`/.well-known/oauth-authorization-server`)
...
In Serverless environments such as Amazon API Gateway, binary WebSocket frames are not supported. The Celerity client SDK automatically detects this via the [server capabilities signal](/docs/framework/runtime/websocket-runtime-protocol#server-capabilities) — no client-side configuration is needed. The client falls back to text-only operation with JSON-format control frames for acknowledgements and lost message notifications.
