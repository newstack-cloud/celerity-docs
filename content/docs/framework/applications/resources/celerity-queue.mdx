...
The maximum number of attempts to process a message before it is moved to the linked dead-letter queue. This is an optional annotation and if not specified, the default value will be used based on the target environment.
This annotation is set on the **parent** (source) queue — the one that defines the `linkSelector` pointing at its dead-letter queue — and configures that queue's redrive behaviour. (On AWS, for example, it maps to the `maxReceiveCount` of the source queue's redrive policy.)
...
### `celerity/bucket` 🔗 `celerity/queue`

The following annotations configure which object storage events (and key filters) from a linked `celerity/bucket` are delivered to the queue.

#### celerity.queue.bucket.events

The object storage events from the linked bucket that should be delivered to the queue.
...
On FaaS/serverless target environments (e.g. `aws-serverless`), the `metadataUpdated` event has no equivalent and is ignored; the queue receives the `created` events by default.
</Callout>

___

#### celerity.queue.bucket.filterPrefix

Restricts notifications delivered to the queue to object keys that begin with the given prefix.
...
#### celerity.queue.bucket.filterSuffix

Restricts notifications delivered to the queue to object keys that end with the given suffix.
...
When a queue links to a topic, messages received by the queue will be published to the topic. Intermediary serverless functions (if necessary) will be provisioned to enable message forwarding from the queue to the topic.
This can be useful if a queue is used as the endpoint of a specific task that should be published to a topic for further processing or to send out notifications. 
...
variable dbPort: number {}
variable encryptionKeyId: string {}

resource orderQueue: celerity/queue {
    metadata {
...
resource orderDLQ: celerity/queue {
    metadata {
...
resource handler: celerity/handler {
    metadata {
