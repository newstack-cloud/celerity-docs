...
If the consumer `sourceId` is a Celerity topic, the consumer will be configured to consume messages from a stream that is prepared for the topic, see [`celerity/topic`](/docs/applications/resources/celerity-topic/current#local-development) for more information on how topics work in the local development environment. If a dead-letter queue is configured, the message will be forwarded to a dedicated Redis stream which can then be inspected through tools such as the Celerity CLI.
