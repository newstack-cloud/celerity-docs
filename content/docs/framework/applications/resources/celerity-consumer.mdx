...
This value is not applicable to the AWS Serverless target — the only v0 deploy target currently available — and is only honoured by container-backed targets planned for a future v0 evolution; see the [configuration mappings](#configuration-mappings) section for more details.
...
This supports NoSQL database streams/events such as DynamoDB Streams, Google Cloud Firestore, and Azure Cosmos DB based where the selected service is based on the target environment.

You can find more information about the configuration mappings for database streams in the [configuration mappings](#serverless-database-streams) section. You can also dive into how DB streams work with containerised and custom server environments [here](/docs/framework/applications/architectures#events---cloud-service-events)
...
See [`celerity/handler`](/docs/framework/applications/resources/celerity-handler#handlers-for-a-message-queue) for integrated examples of how to use consumers for an application.
...
When a consumer defines `externalEvents`, each entry is wired as an event source mapping (or notification) for the handler's Lambda function: a `dbStream` maps to a DynamoDB Streams event source mapping (`dbStreamId` → `eventSourceArn`), a `dataStream` to a Kinesis event source mapping (`dataStreamId` → `eventSourceArn`), and an `objectStorage` source to an S3 event notification on the referenced bucket. Where supported, `batchSize`, `partialFailures` (→ `functionResponseTypes`) and `startFromBeginning` (→ `startingPosition = "TRIM_HORIZON"`) are applied to the mapping. The same event-source-mapping wiring applies when `sourceId` refers to an external Amazon SQS queue that is not defined in the blueprint.
...
Google Cloud Firestore event triggers aren't actually stream-based, but comes under database streams as is the closest analogue to DynamoDB Streams and Azure Cosmos DB Triggers.
...
<table>
    <thead>
        <tr>
            ...
            <th>Google Cloud Firestore Trigger for Cloud Functions</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>`{database}(:{collectionGroup})?` (Maps to event filters)</td>
...
This is used when the target environment is `azure` and [`azure.compute.containerService`](/docs/framework/applications/compute-configuration#azurecomputecontainerservice) is set to `containerApps`.
...
[^2]: Examples of Serverless stream flows include [Amazon DynamoDB Streams and AWS Lambda](https://docs.aws.amazon.com/lambda/latest/dg/with-ddb.html), [Google Cloud Firestore triggering Google Cloud Functions](https://cloud.google.com/functions/docs/calling/cloud-firestore) and [Azure Cosmos DB Streams triggering Azure Functions](https://learn.microsoft.com/en-us/azure/azure-functions/functions-bindings-cosmosdb?toc=%2Fazure%2Fcosmos-db%2Ftoc.json&bc=%2Fazure%2Fcosmos-db%2Fbreadcrumb%2Ftoc.json&tabs=csharp).
