...
When set to `true`, allows destructive schema changes (dropping tables, dropping columns). Without this annotation, Celerity's analysis of pending migrations — both generated and hand-written — blocks destructive statements: `celerity schema diff` will not generate them, and applying a migration containing them fails with a warning.
