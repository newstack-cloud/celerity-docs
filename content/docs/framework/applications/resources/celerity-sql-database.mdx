...
The schema file defines the desired state of the database (tables, columns, indexes, constraints, extensions) in a declarative YAML format. When the schema changes, `celerity schema diff` computes the difference between the current and desired state (powered by the embedded open-source [Atlas](https://atlasgo.io) engine) and generates a versioned SQL migration file capturing the transition.
...
The path to the migration directory for the database, relative to the blueprint file. This single directory holds the full linear migration history: files generated from schema YAML changes by `celerity schema diff` and hand-written files for operations the schema YAML cannot express (custom functions, triggers, row-level security policies, partitioning).

Files follow the naming convention `<timestamp>_<description>.sql` and are executed in timestamp order. An `atlas.sum` integrity file guards against tampered or inconsistent directories. Applied migrations are tracked in a revision table (`atlas_schema_revisions`) in each managed database to ensure idempotency.

See [Migration Directory](/docs/framework/applications/sql-database-schema-management#migration-directory) for details.
...
| Value | Description |
|---|---|
| `password` | Static username/password credentials. Celerity auto-generates credentials during provisioning and stores them in the secret store or environment variables. |
| `iam` | Platform IAM identity authentication. The handler's execution role, service account or managed identity is granted database access. The SDK generates short-lived tokens at runtime — no static passwords are stored. |

| Value | AWS | Google Cloud | Azure |
|---|---|---|---|
| `password` | Static credentials via Secrets Manager | Static credentials | Static credentials |
| `iam` | [IAM Database Authentication](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/UsingWithRDS.IAMDBAuth.html) (STS tokens) | [IAM Database Authentication](https://cloud.google.com/sql/docs/postgres/iam-authentication) (OAuth2 tokens via Cloud SQL Connector) | [Entra ID Authentication](https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/concepts-azure-ad-authentication) (managed identity tokens) |
...
Destructive schema changes (dropping tables or columns) are not gated by a resource annotation. Instead, `celerity schema diff` generates the migration and classifies it `breaking`, and `celerity schema apply` will not apply a `breaking` migration without `--allow-breaking`. This is enforced by an un-spoofable floor that inspects each migration's actual SQL, so a hand-edited risk header cannot bypass it. See [Migration Risk Classification](/docs/framework/applications/sql-database-schema-management#migration-risk-classification).
...
The hostname or endpoint for read replica connections. Only present when `readReplicas` is enabled.

On Aurora Serverless v2 this is the cluster reader endpoint, which load-balances connections across all reader instances in the cluster. On standalone Amazon RDS, this resolves to the read-replica instance endpoint; load-balancing a single reader endpoint across multiple standalone replicas is 🔄 **Planned for v0**.
...
resource ordersVpc: celerity/vpc {
    metadata {
...
resource getOrderHandler: celerity/handler {
    metadata {
...
2. Migration files are applied in timestamp order, skipping migrations already tracked in the revision table
3. Connection environment variables are injected into your handlers
...
3. Migration directory integrity is verified and a pre-apply drift check runs, then pending migration files are applied in order
...
When `readReplicas` is enabled, Celerity provisions Amazon RDS read replicas for horizontal read scaling. Same-region replicas default to 1 and can be configured via `aws.rds.<dbName>.readReplicas.count` in the deploy config. In v0 the `readHost` output resolves to the read-replica instance endpoint; a single load-balanced reader endpoint across all standalone RDS replicas is 🔄 **Planned for v0**. The reader endpoint is surfaced as the `readHost` output and through the SDK credentials provider.
...
When `authMode` is set to `"iam"`, Celerity enables [IAM Database Authentication](https://cloud.google.com/sql/docs/postgres/iam-authentication) on the Cloud SQL instance. The handler's service account is registered as a Cloud SQL IAM user and granted `roles/cloudsql.client` and `roles/cloudsql.instanceUser`. The SDK uses the Cloud SQL Connector to generate OAuth2 access tokens at runtime.
...
When `authMode` is set to `"iam"`, Celerity enables [Entra ID Authentication](https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/concepts-azure-ad-authentication) on the Azure Database server. The handler's managed identity is granted database access, and the SDK fetches access tokens from Entra ID at runtime.
...
For language-specific documentation, see [Node.js SDK - SQL Database](/docs/node-runtime/sql-database) and [Python SDK - SQL Database](/docs/python-runtime/sql-database).
...
Celerity provides a built-in schema management system for SQL databases that makes database schemas a first-class concern. Schema definitions are written in declarative YAML, and schema changes become generated, reviewable SQL migration files — powered by the embedded open-source [Atlas](https://atlasgo.io) engine.

Key capabilities include:
- **Declarative schema definition** in YAML with engine-native column types
- **Generated migrations** — safe, reviewable DDL transitions produced by `celerity schema diff`
- **Hand-written migrations** in the same directory for operations the schema YAML cannot express
- **Drift detection** for surfacing out-of-band database changes
