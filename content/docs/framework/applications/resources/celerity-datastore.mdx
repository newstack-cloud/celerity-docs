...
When `schemaPath` is provided, the external schema file takes precedence and this inline schema is ignored. The inline schema always describes a single item shape; [multi-entity schemas](/docs/framework/applications/nosql-datastore-schema-management#multiple-entities-single-table-design) for single-table designs are supported in external schema files only.
...
The path to an external YAML schema definition file, relative to the blueprint file. When provided, Celerity manages the datastore schema through its [built-in schema management](/docs/framework/applications/nosql-datastore-schema-management) system. The external schema file supports rich metadata (`owner`, `tags`, `classification`, `default`) and [multiple entities for single-table designs](/docs/framework/applications/nosql-datastore-schema-management#multiple-entities-single-table-design), and integrates with schema contracts, type generation, drift detection and export systems.
...
Scripts follow the naming convention `V<number>__<description>.<ext>` and can be written in any language. They are tracked in schema state and surfaced in `celerity schema diff`, but are NOT automatically executed during deployment; run them with [`celerity schema scripts run`](/docs/cli/reference#scripts-run), which also records their completion in schema state. See [Escape Hatch Data Scripts](/docs/framework/applications/nosql-datastore-schema-management#escape-hatch-data-scripts) for details.
...
### celerity.datastore.validationMode

Controls how the Celerity SDK validates items against the schema on writes at runtime. Only applies when a schema is defined for the data store.
...
The `warn` default means tightening a schema (such as making a field required) can never break production writes for items that predate the change. Since the annotation is per resource, `off` serves as an escape hatch for extreme high-throughput write paths without disabling validation for other datastores; drift scans still observe the datastore's data shape from the outside. See [Runtime Validation](/docs/framework/applications/nosql-datastore-schema-management#runtime-validation-sdk-abstraction) for the full model.
...
No annotations are required for linking other resources to a `celerity/datastore` resource.
...
A default value for the field. Defaults are applied at item construction time by generated code (Pydantic field defaults in Python, an exported defaults constant in TypeScript) and are never silently injected by the SDK on write, nor enforced by the database. See [Defaults](/docs/framework/applications/nosql-datastore-schema-management#defaults) for details.
...
resource userStore: celerity/datastore {
    ...
    spec {
        ...
        indexes = [
            { name = "emailNameIndex", fields = ["email", "name"] }
        ]
    }
}
```
...
resource userStore: celerity/datastore {
    ...
    spec {
        ...
        indexes = [
            ...
            { name = "teamRoleIndex", fields = ["teamId", "role"] }
        ]
        timeToLive = {
            fieldName = "ttl",
            enabled = true
...
For language-specific documentation, see [Node.js SDK - Datastore](/docs/node-runtime/datastore) and [Python SDK - Datastore](/docs/python-runtime/datastore).
...
- **Schema-as-code**: Declarative YAML definitions with rich metadata (`description`, `owner`, `tags`, `classification`) that are version-controlled alongside your application, including [multi-entity schemas](/docs/framework/applications/nosql-datastore-schema-management#multiple-entities-single-table-design) for single-table designs
...
- **Schema validation**: Layered validation — runtime (SDK validates on writes, in `warn` mode by default per the [`validationMode` annotation](#celeritydatastorevalidationmode)), build-time (generated types) and CI (`celerity schema validate`)
- **Drift detection**: Sampled scans of live data report per-field conformance against the schema and surface unknown fields (`celerity schema drift`)
...
- **Data scripts**: Versioned escape hatch scripts for backfills and data transforms, run and tracked via `celerity schema scripts run`
