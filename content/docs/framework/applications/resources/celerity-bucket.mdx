...
Which events are delivered (and any object key prefix/suffix filters) is configured with the `celerity.queue.bucket.*` annotations set on the receiving queue.
...
Which events are delivered (and any object key prefix/suffix filters) is configured with the `celerity.topic.bucket.*` annotations set on the receiving topic.
...
🚀 **Planned for v1** - Bucket to workflow links are planned for v1.
...
resource myAppBucket: celerity/bucket {
    metadata {
            ...
            rules = [
                {
                    ...
                    transitions = [
                        { targetStorageClass = "cool", transitionInDays = 30 }
                    ]
                }
            ]
        }
...
In the local development environment, object storage is provided by a [minio](https://min.io/) instance running on a container network on the local or CI machine.
...
[valkey](https://github.com/valkey-io/valkey) is also used as the `celerity/queue` and `celerity/topic` implementation in the local development environment, so the same behaviour is mostly reused for bucket notifications.
</Callout>
