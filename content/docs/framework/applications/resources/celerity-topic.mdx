...
No annotations are required to modify the behaviour of a topic resource itself.

`linkSelector.byLabel` can be used to target topics from other resource types.

### `celerity/bucket` 🔗 `celerity/topic`

The following annotations configure which object storage events (and key filters) from a linked `celerity/bucket` are delivered to the topic.

#### celerity.topic.bucket.events

The object storage events from the linked bucket that should be delivered to the topic.
...
On FaaS/serverless target environments (e.g. `aws-serverless`), the `metadataUpdated` event has no equivalent and is ignored; the topic receives the `created` events by default.
</Callout>

___

#### celerity.topic.bucket.filterPrefix

Restricts notifications delivered to the topic to object keys that begin with the given prefix.
...
#### celerity.topic.bucket.filterSuffix

Restricts notifications delivered to the topic to object keys that end with the given suffix.
...
resource orderEventsTopic: celerity/topic {
    metadata {
...
resource createOrderHandler: celerity/handler {
    metadata {
...
variable encryptionKeyId: string {
...
For language-specific SDK documentation, see [Node.js SDK](/docs/node-runtime) and [Python SDK](/docs/python-runtime). Topic-specific SDK pages are coming soon.
