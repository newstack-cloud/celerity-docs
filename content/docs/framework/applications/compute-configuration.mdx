...
description: Configuration options for compute resources in Celerity applications
---
...
- **v1 (Preview)**: Multi-cloud deployments for containerised applications running in the Celerity runtime.

Read more about Celerity versions [here](/docs/framework/versions).
</Callout>

🚀 **Planned for v1** - Support for compute configuration is planned for v1, a subset of the configuration options will become available in future v0 evolutions. This document will be updated to reflect these changes.

Celerity applications can be composed of multiple compute resources that represent different interfaces for an application. These compute resources share common configuration specific to a deployment target that can be defined in the [app deploy configuration file](/docs/cli/app-deploy-configuration).
