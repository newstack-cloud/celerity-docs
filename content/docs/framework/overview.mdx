...
For **SQL databases**, the SDK provides a credentials provider that abstracts connection configuration sourcing, plus an optional [knex.js](https://knexjs.org/) connection factory with managed connection pooling. You bring your own SQL library or ORM — the SDK handles connection lifecycle, not queries. For **NoSQL datastores**, the SDK provides a cloud-agnostic interface for common CRUD operations. For **caches**, the SDK provides an abstraction covering all core Redis data structures (strings, hashes, lists, sets, sorted sets), with a credentials provider available for direct Redis client access when you need advanced features like pub/sub, streams or transactions.
</Callout>
