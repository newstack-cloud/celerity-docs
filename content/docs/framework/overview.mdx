...
### Product & data team friction

- **Pain point**: The teams that build a product and the teams that analyse its data are constantly out of sync. The data model is scattered across application code and tribal knowledge, and with NoSQL stores there is no database-level schema to introspect at all, so data teams build pipelines effectively blind. When product teams evolve the schema, breaking changes surprise downstream consumers with no warning, and keeping documentation, types and pipelines aligned is a manual, error-prone effort.
- **Solution**: Celerity makes the schema a first-class, version-controlled source of truth shared by both teams. [Schema-as-code](/docs/framework/applications/nosql-datastore-schema-management) YAML describes the intended structure with rich ownership and classification metadata; [type generation](/docs/framework/applications/nosql-datastore-schema-management#type-generation) keeps application code type-safe in supported languages; [schema contracts](/docs/framework/applications/nosql-datastore-schema-management#schema-contracts) let data teams declare the dependencies they rely on so breaking changes are flagged in CI before they ship; [drift detection](/docs/framework/applications/nosql-datastore-schema-management#drift-detection) surfaces where live data or databases have diverged from the schema; and [data team exports](/docs/framework/applications/nosql-datastore-schema-management#for-data-teams) (Markdown, JSON Schema, Avro and Mermaid ERD) feed documentation and pipeline tooling directly. See the [NoSQL datastore](/docs/framework/applications/nosql-datastore-schema-management) and [SQL database](/docs/framework/applications/sql-database-schema-management) schema management guides for the full picture.
...
The API building block ([`celerity/api`](/docs/framework/applications/resources/celerity-api)) is used to define a HTTP or WebSocket API that exposes functionality of your application to frontends and other systems. This comes with support for authentication and CORS out of the box. Celerity WebSocket APIs come with built-in resilience and a client SDK to make interacting with them as smooth as possible; the built-in support for authentication and CORS extends to WebSocket APIs.

[Read more about Celerity APIs](/docs/framework/applications/resources/celerity-api).

### Channel

The channel building block ([`celerity/channel`](/docs/framework/applications/resources/celerity-channel)) is an opt-in layer on top of WebSocket APIs that provides real-time pub/sub messaging, presence tracking, per-channel authentication and rate limiting. Channels are declared as resources in the blueprint and linked from a `celerity/api` with the WebSocket protocol. The runtime handles subscription management, message fan-out to all subscribers, and cross-node delivery in cluster deployments. A client SDK (`@celerity-sdk/channels`) wraps the WebSocket transport client to provide channel subscription, presence events, and automatic re-subscription on reconnect.
...
The SQL database building block ([`celerity/sql-database`](/docs/framework/applications/resources/celerity-sql-database)) is used to define a SQL database that can be used to store structured data. This includes support for multiple providers such as AWS RDS, Azure SQL Database and GCP Cloud SQL. Celerity provides [built-in schema management](/docs/framework/applications/sql-database-schema-management) for SQL databases, covering schema-as-code with generated, reviewable migrations (powered by the open-source core of [Atlas](https://atlasgo.io)), type generation, schema contracts, drift detection and data team exports, giving development and data teams a single, versioned source of truth for the database structure.

[Read more about Celerity SQL Databases](/docs/framework/applications/resources/celerity-sql-database).

### NoSQL Database

The NoSQL database building block ([`celerity/datastore`](/docs/framework/applications/resources/celerity-datastore)) is used to define a NoSQL database that can be used to store unstructured data. This includes support for multiple providers such as AWS DynamoDB, Azure Cosmos DB and GCP Firestore. Celerity provides [built-in schema management](/docs/framework/applications/nosql-datastore-schema-management) for NoSQL datastores — including schema-as-code, type generation, schema contracts, data-shape drift detection and data team exports — bridging the gap where NoSQL databases have no database-level schema to introspect.
...
The Celerity CLI is a delightful command-line tool to manage your Celerity applications. It provides a set of interactive commands to help create, test and deploy your applications. The CLI also includes local development environment tooling that uses open source software to power infrastructure primitives such as databases, queues and caches for running and testing your applications locally.

[Read more about the Celerity CLI](/docs/cli/getting-started).
...
The Celerity SDKs are a set of libraries for the languages that Celerity supports to help you build your applications.
...
<Callout title="Database and Cache Packages" type="info">
For **SQL databases**, the SDK provides a credentials provider that abstracts connection configuration sourcing, plus an optional connection factory with managed connection pooling. You bring your own SQL library or ORM — the SDK handles connection lifecycle, not queries. For **NoSQL datastores**, the SDK provides a cloud-agnostic interface for common CRUD operations. For **caches**, the SDK provides an abstraction covering all core Redis data structures (strings, hashes, lists, sets, sorted sets), with a credentials provider available for direct Redis client access when you need advanced features like pub/sub, streams or transactions.
</Callout>

Celerity provides SDKs for the following languages. In v0, Node.js (TypeScript) and Python are available; Go, C#/.NET and Java are planned for v1 (see [Celerity versions](/docs/framework/versions)):

- [Node.js (TypeScript)](/docs/node-runtime) — ✅ Available in v0
- [Python](/docs/python-runtime) — ✅ Available in v0
- [Go](/docs/go-sdk) — 🚀 Planned for v1
- [C#/.NET](/docs/csharp-runtime) — 🚀 Planned for v1
- [Java](/docs/java-runtime) — 🚀 Planned for v1

### Bluelink

[Bluelink](https://www.bluelink.dev) is an infrastructure management tool that is used to deploy and manage the infrastructure for your Celerity applications. It provides the [configuration format for blueprints](https://www.bluelink.dev/docs/bluelink/blueprints/specification) that are used to define the components of your application. Bluelink has a unique approach that extends the concept of [infrastructure as code](https://en.wikipedia.org/wiki/Infrastructure_as_code) to a concept that can be referred to as "infrastructure as relationships" that enables you to focus on the important infrastructure components and connect them together without having to define networking, permissions and other "glue" elements.
...
[^1]: In the current plan for v1, Celerity only aims to support AWS, Azure, GCP and local environments. However, the goal is to support on-premise environments in the future.
