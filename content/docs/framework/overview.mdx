...
<Callout title="Database and Cache SDKs" type="info">
For **SQL databases**, the SDKs provide a credentials provider that abstracts connection configuration sourcing, plus an optional [knex.js](https://knexjs.org/) connection factory with managed connection pooling. You bring your own SQL library or ORM — the SDK handles connection lifecycle, not queries. For **NoSQL datastores**, the SDKs provide a cloud-agnostic interface for common CRUD operations. For **caches**, the SDKs provide a thin abstraction (`get`/`set`/`delete`/`ttl`) over Redis, with a credentials provider available for direct Redis client access when you need the full Redis API.
</Callout>
