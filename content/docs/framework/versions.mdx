...
- `celerity/workflow`
    ...
    - Azure Container Apps or AKS for the Azure deployment target running the Celerity workflow runtime with Azure Cosmos DB used for workflow state persistence via the Cassandra API for Cosmos DB (providing a degree of parity with the local development environment).
