...
- **Celerity Core Runtimes & SDKs**: Node.js and Python runtimes only, supporting HTTP APIs, WebSocket APIs, consumers, schedules and workflows.
- **Celerity Workflow Runtime**: Workflow orchestration with support for Node.js and Python runtimes, available for local development and AWS serverless (Step Functions) deployments.
...
- `celerity/workflow` - AWS Step Functions for the AWS Serverless deployment target and the Celerity workflow runtime locally.
