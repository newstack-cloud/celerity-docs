...
**Initial Release**: October 30th, 2026
**Evolution Period**: October 2026 - October 2027
...
#### Current v0 Components (October 2026)
- **Celerity CLI**: Full set of commands for starting projects along with the development and deployment lifecycle.
- **Celerity Core Runtimes & SDKs**: Node.js and Python runtimes only, supporting HTTP APIs, WebSocket APIs, consumers, schedules, SQL databases, NoSQL datastores and caches.
- **Client SDK for WebSocket APIs**: A single TypeScript package for browsers and Node.js to interact with WebSocket APIs adhering to the [WebSocket Runtime Protocol](/docs/framework/runtime/websocket-runtime-protocol), that are also compatible with AWS API Gateway v2 WebSocket APIs.
- [**Bluelink Deploy Engine**](https://bluelink.dev/docs/bluelink/overview): The engine that powers deployments of Celerity applications, initially supporting only a subset of AWS services that enable serverless deployments.
- [**Bluelink Blueprint Language Server**](https://bluelink.dev/docs/bluelink/overview): Diagnostics, validation and auto-complete support for blueprints used to define Celerity applications.

#### Current v0 Resource Types (October 2026)
...
- `celerity/datastore` - AWS DynamoDB tables with built-in schema management (schema-as-code, type generation, schema contracts, data-shape drift detection, data team exports).
...
- `celerity/sqlDatabase` - AWS RDS for PostgreSQL with built-in schema management (schema-as-code with generated migrations, drift detection, type generation).
- `celerity/cache` - AWS ElastiCache for Redis with an SDK abstraction covering all core Redis data structures (strings, hashes, lists, sets, sorted sets), atomic counters, transactions and a credentials provider for direct Redis client access.

#### Cloud Provider Support
- **AWS only** (initially)

### v1 (Preview) - Full Capabilities
**Target Release**: October 2027
...
- **Client SDK for WebSocket APIs**: Client SDKs for browsers, Node.js and Python to interact with WebSocket APIs adhering to the [WebSocket Runtime Protocol](/docs/framework/runtime/websocket-runtime-protocol), that are also compatible with AWS API Gateway v2 WebSocket APIs. v1 adds a Python WebSocket client SDK.
- [**Bluelink Deploy Engine**](https://bluelink.dev/docs/bluelink/overview): The engine that powers deployments of Celerity applications, supporting AWS, Azure and Google Cloud.
- [**Bluelink Blueprint Language Server**](https://bluelink.dev/docs/bluelink/overview): Diagnostics, validation and auto-complete support for blueprints used to define Celerity applications.
...
- `celerity/workflow`
    ...
    - Azure Container Apps or AKS for the Azure deployment target running the Celerity workflow runtime with Azure Cosmos DB used for workflow state persistence via the NoSQL API for Cosmos DB.
    - Google Cloud Workflows for the Google Cloud Serverless deployment target.
    - Google Cloud Run or GKE for the Google Cloud deployment target running the Celerity workflow runtime with Google Cloud Firestore used for workflow state persistence.
...
- `celerity/sqlDatabase`
    - AWS RDS for PostgreSQL for the AWS deployment targets. (Available in v0)
    - v1 adds MySQL support and multi-cloud:
...
- `celerity/cache`
    - AWS ElastiCache for Redis for the AWS deployment targets. (Available in v0)
    - v1 adds multi-cloud:
...
### v1+ (Future) - Post-v1 Features

v1+ represents features that are planned for after the v1 release in July 2027. These are features that have been designed and or considered of but are not part of the v1 roadmap.

#### Planned Resource Types

- `celerity/channel` - An opt-in real-time channel layer on top of WebSocket APIs providing pub/sub messaging, presence tracking, per-channel auth, and rate limiting. Channels are declared as resources in the blueprint and linked from a `celerity/api` with the WebSocket protocol. The runtime handles subscription management, message fan-out, and cross-node delivery in cluster deployments. A client SDK (`@celerity-sdk/channels`) provides channel subscription, presence, and automatic re-subscription on reconnect.

#### Planned Provider Support

- **Self-hosted/on-premise** - Support for self-hosted deployments is planned for v1+ with a focus on a Kubernetes-based deployment of the Celerity stack, control plane and your workloads powered by open source software.
...
### Simple Timeline
1. **Current**: `v2026-02-27-draft` (single evolving spec)
2. **v1 Release**: `v2027-10-30` (stable, no draft suffix)
